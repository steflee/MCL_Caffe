 ...
 */
template <typename Dtype>
class EnsembleAccuracyLayer : public Layer<Dtype> {
 public:
  explicit EnsembleAccuracyLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "EnsembleAccuracy"; }
...
class OracleAccuracyLayer : public Layer<Dtype> {
 public:
  ...
  virtual inline const char* type() const { return "OracleAccuracy"; }
  virtual inline int MinNumBottomBlobs() const { return 2; }
  virtual inline int ExactNumTopBlobs() const { return 1; }
